...
const SingleTodo = ({todo, todos, setTodos}: SingleTodoProps) => {
  ...
  return (
    <form action="" className='todos__single' onSubmit={(ev) => handleEdit(ev, todo.id)} >
      {
        edit? (
          <input type="text" value={editTodo} onChange={(ev) => setEditTodo(ev.target.value)} className='todos__single--text' ref={editInputRef} />
        ): (
